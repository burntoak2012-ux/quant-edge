...
export default function SignalsPage() {
  ...
  {signals.map((signal) => (
    <Link key={signal.id} href={`/signals/${signal.id}`} className="block">
      <div className="border rounded-2xl p-5 shadow-sm bg-white hover:shadow-md transition cursor-pointer">
        ...
        <p className="text-sm mt-3">
          Prediction: <span className="font-medium">{signal.prediction}</span>
        </p>
      </div>
    </Link>
  ))}
